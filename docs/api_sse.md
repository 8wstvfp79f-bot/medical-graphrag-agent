# FastAPI `/chat`, Async Retrieval and SSE

## 这一阶段做了什么

Phase 8 把前七个阶段的“证据流水线”变成一个真正可调用的问答接口：
...
请求 schema 校验
   ↓
疾病实体 + 意图 + strict tool call
   ↓
Milvus 文本检索 ─┐
                  ├─ asyncio 并发 → RRF → BGE-Reranker → Context Budget
Neo4j 图谱检索 ──┘
   ↓
抽取式带引用回答
   ↓
...
| 文件 | 作用 |
| --- | --- |
| `src/api/schemas.py` | 定义 `/chat` 请求和响应结构；限制字符串长度、Top-K 1–50、metadata 字段与未知字段 |
| `src/api/service.py` | 串联规划、异步 Agent、答案生成与响应组装；为每次请求生成 `request_id` |
| `src/api/runtime.py` | 服务启动时一次性加载本地 BGE、Reranker、Milvus、Neo4j 和疾病词典；关闭时释放连接 |
| `src/api/app.py` | 提供 `/health`、`/chat`、OpenAPI 文档和 SSE 编码 |
| `src/generation/answer_generator.py` | 定义可替换的 `AnswerGenerator`；既支持本地抽取式回答，也支持带引用校验的真实兼容Chat模型 |
| `src/llm/client.py` | 调用本地或托管的OpenAI兼容Chat接口；密钥只进入请求头，不写入结果 |
| `src/retrieval/hybrid_retriever.py` | `hybrid_search_async` 同时启动 Vector 与 Graph 分支，完成后统一融合 |
...
`disease_name` 可由问题自动识别，也可在请求顶层显式传入。Hybrid 计划会把最终疾病名同步到向量 metadata filter 与图谱参数中。若没有疾病名或同时识别出多个独立疾病，接口返回 `needs_clarification`，不会访问数据库。
...
| `answer_generation` | 当前答案方式和实际使用的 evidence IDs；目前是 `extractive_fallback` |
| `sources` / `citations` | 原始来源列表，以及 E1-En 到 evidence 的对应关系 |
| `evidence` | Context Budget 最终允许进入回答层的证据 |
...
| `context_budget` | 上下文使用了多少估算 token、选了几条证据 |
...
默认仍使用不需要密钥的抽取式后端。配置真实兼容Chat服务后可切换到带引用约束的LLM生成：
...
模型只能看到Context Budget选出的E1-En证据。返回后会再次校验引用编号；无引用、空回答或引用越界时，默认退回抽取式答案，响应中的`answer_generation.mode`会说明实际使用了哪条路径。
...
真实LLM生成适配器已经实现，但只有在配置模型地址并且响应中的`mode=llm_grounded`时，才能说明该次请求由LLM生成。当前提交中的正式批量评测仍使用抽取式后端；外部Judge结果也必须单独运行后才能对外报告。
