...
| --- | ---: |
| Documents | 8,807 |
| Core triples | 182,565 |
| Candidate triples（独立候选池） | 548,982 |
| Core + candidate triples | 731,547 |
| Core entities | 21,604 |
| Core disease entities | 8,806 |
| All unique text entities | 259,912 |
...
| Output | Scale | Main fields |
| --- | ---: | --- |
| `data/processed/documents.jsonl` | 8,807 | `doc_id`, `disease_name`, `source`, `text`, `metadata` |
| `data/processed/chunks.jsonl` | 20,801 | chunk text、BGE 检索字段和 metadata |
| `data/processed/triples.csv` | 182,565 | `head`, `relation`, `tail`, `source` |
| `data/processed/candidate_triples.csv` | 548,982 | 与核心图谱隔离的候选关系池 |

文档与图谱构建共用 `src/data/medical_quality.py` 和版本化配置 `config/medical_data_quality.json`。清洗审计共拒绝 243 个核心字段值；已确认的污染症状实体在最终 documents、chunks 和 triples 中均为 0。详细 Bad Case 与数据库同步过程见 `data_quality.md`。

## Relation Distribution

| Relation | Count | Neo4j purpose |
| --- | ---: | --- |
...
| **Total** | **182,565** | 核心医学关系主链路 |

## Core Relations vs. Candidate Relations

Core relations 是当前 Neo4j 主链路直接使用的五类医学关系：症状、检查、药品、科室和并发症。它们结构稳定、可解释性强，适合从疾病实体出发做一跳或受控多跳扩展，并可在回答中作为证据类型展示。

Candidate relations 是可从原始字段继续派生、但默认不进入主图的关系，例如：
...
## Neo4j Ingestion Policy

实际 Neo4j 主链路优先导入 Core relations。这样可以控制节点/边规模、减少长文本伪实体与低价值关系造成的图扩展噪声，并让 `graph_search` 的 evidence 类型保持清晰。候选关系应进入独立实验标签或灰度 collection，不应为了扩大关系边数量直接混入生产查询路径。
