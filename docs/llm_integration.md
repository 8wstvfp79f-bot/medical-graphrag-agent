# 真实LLM生成与LLM-as-a-Judge

## 当前状态

代码已经支持真实的OpenAI兼容Chat接口，可以连接本地LM Studio、Ollama兼容网关或托管模型服务。当前机器已通过LM Studio接入`qwen2.5-7b-instruct`，完成500个真实回答和500/500条Judge评分。实测Vector-only/Hybrid语义幻觉题目占比为49.6%/29.6%，绝对下降20.0个百分点、相对下降40.3%。该结果来自模型评审，不等于临床专家结论。

## 两个模型角色

生成模型和Judge应尽量使用不同配置，避免“自己生成、自己打分”：

- Answer Generator：读取Context Budget内的E1-En证据，生成带引用回答；
- LLM Judge：读取问题、回答和证据，对Faithfulness、Relevance、Completeness评分，并列出Unsupported Claims。
...
当Judge列出至少一个`unsupported_claim`，或Faithfulness低于配置阈值时，该题计为“出现语义幻觉”。
...
只有真实Judge完成Vector和Hybrid两组评分，且Vector幻觉率大于0时，才会输出相对下降比例。本次250组配对问题、500条评分全部成功，实测相对下降40.3%；不能修改阈值、删题或继续写预设的35%来迎合目标。

当前Answer Generator与Judge都使用`qwen2.5-7b-instruct`。这能验证完整本地模型链路，但同模型自评可能存在偏差；固定测试集也来自本项目清洗数据，尚未经过独立临床专家标注。更强的对外结论应再用不同Judge模型和人工抽样复核。
